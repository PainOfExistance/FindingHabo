...
class PlayerMenu:
    ...
    def render_inventory(self):
        ...
        for item_name, item_quantity in self.player.inventory.quantity.items():
            item_text = f"{item_name}: {item_quantity} desc: {self.player.inventory.items[item_name]['description']} "
            print(item_text)
            item_render = inventory_font.render(item_text, True, (44, 53, 57))
            item_rect = item_render.get_rect(topleft=(220, 20))
    ...
    def render(self):
        if self.visible:
            ...
            for index, item in enumerate(self.menu_items):
                color = (44, 53, 57) if index == self.selected_item else (237, 106, 94)
